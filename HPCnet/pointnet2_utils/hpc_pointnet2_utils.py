...
from tools.HausdorffTest.getGtFeature import get_gt_feature
...
class HPC_Group(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor, new_xyz: torch.Tensor, features: torch.Tensor = None) -> Tuple[torch.Tensor]:
        ...
        if features is not None:
            grouped_features = grouping_operation(features, idx)  # (B, C, npoint)
            if self.use_xyz:
                new_features = torch.cat([grouped_xyz[:,:,:,0], \
                                          gtfeatures, grouped_features[:,:,:,0]], \
                                         dim=1)  # (B, C + 3 + 42, npoint, nsample) 我觉得这里应该cat new_xyz 而不是 new_xyz每个点找的nsamle中的第一个（第一个不一定是new_xyz自己）
            else:
...
class GroupAll(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor, new_xyz: torch.Tensor, features: torch.Tensor = None):
        ...
        if features is not None:
            ...
            if self.use_xyz:
                new_features = torch.cat([grouped_xyz, grouped_features], dim=1)  # (B, 3 + C, N)
            else:
                new_features = grouped_features
        else:
            new_features = grouped_xyz
